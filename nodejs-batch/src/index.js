 ...
 * @returns {Promise<boolean>} ユーザーが承認した場合true
 */
...
/**
 * メイン実行関数
 * コマンドライン引数を解析し、レジストリデータの処理やマイグレーションを実行する
...
async function main() {
  ...
  try {
    ...
    if (argv.migrate) {
      ...
      const confirmed = await askConfirmation('この操作を続行しますか？ (y/N): ');
    ...
    if (argv.registry) {
      ...
      const result = await finalProcessor.processRegistry(argv.registry, processOptions);
    ...
    // 全レジストリ処理
      ...
      if (argv.parallel) {
        ...
        for (const registry of registries) {
          const result = await finalProcessor.processRegistry(registry, processOptions);
          results.push(result);
...
/**
 * レジストリデータをトランザクションテーブルから本テーブルに移行する
 * トランザクションで安全にデータを移行し、既存データは削除される
 * @param {DatabaseManager} dbManager - データベースマネージャー
 * @param {string} registry - レジストリコード
 * @returns {Promise<void>}
 * @throws {Error} データ移行エラー時
 */
async function finalizeRegistry(dbManager, registry) {
  ...
  try {
    ...
    // 本テーブルの既存データを削除
    await dbManager.connection.execute('DELETE FROM ip_allocations WHERE registry = ?', [registry]);
...
/**
 * 処理結果をコンソールに表示する
 * 各レジストリの処理結果をサマリー形式で表示
 * @param {Array<Object>} results - 処理結果の配列
 * @param {string} results[].registry - レジストリコード
 * @param {boolean} results[].success - 処理成功フラグ
 * @param {number} [results[].recordsProcessed] - 処理したレコード数
 * @param {number} [results[].executionTime] - 実行時間（秒）
 * @param {string} [results[].error] - エラーメッセージ
 */
